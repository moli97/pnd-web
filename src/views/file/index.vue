  ...
  <div class="file-list-container">
    <div class="operater">
      ...
      <el-button-group class="extend-btn">
        <el-button icon="el-icon-edit" v-if="selection.length === 1" @click="renameFile(selection[0])" plain>重命名
        </el-button>
        <el-button icon="el-icon-download" v-if="canDownload()" @click="downloadFiles(selection)" plain>下载</el-button>
        <el-button icon="el-icon-delete" v-if="selection.length > 0" plain @click="deleteFiles(selection)">删除
        </el-button>
            ...
            <el-dropdown>
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item @click.native="downloadFiles([scope.row])"
                                  v-if="getFileType(scope.row.type) != 'folder'">下载
                </el-dropdown-item>
    ...
    <div>
      <el-dialog :title="filePreview.title" :visible.sync="filePreview.show" width="100%"
                 style="max-width: 1000px;margin: auto;min-height: 500px">
        <el-button v-if="filePreview.rawType == 'web'" @click="webOutputFormat" size="mini"
                   style="position: relative; left: 4px">change
        </el-button>
        <div>
          ...
          <iframe v-else-if="filePreview.type == 'web'" :srcdoc="filePreview.code" style="width: 100%;height: 100%;"></iframe>
        </div>
      </el-dialog>
    </div>
